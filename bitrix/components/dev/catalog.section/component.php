...
$numPage = $arResult["NAV_RESULT"]->PAGEN;
if($arParams["SET_TITLE"])
{
	if ($arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"] != "")
		$APPLICATION->SetTitle(($numPage>1?'Страница №'.$numPage.'. ':'').$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"], $arTitleOptions);
	elseif(isset($arResult["NAME"]))
		$APPLICATION->SetTitle(($numPage>1?'Страница №'.$numPage.'. ':'').$arResult["NAME"], $arTitleOptions);
}
...
if ($arParams["SET_BROWSER_TITLE"] === 'Y')
{
	...
	if (is_array($browserTitle))
		$APPLICATION->SetPageProperty("title", ($numPage>1?'Страница №'.$numPage.'. ':'').implode(" ", $browserTitle), $arTitleOptions);
	elseif ($browserTitle != "")
		$APPLICATION->SetPageProperty("title", ($numPage>1?'Страница №'.$numPage.'. ':'').$browserTitle, $arTitleOptions);
}
