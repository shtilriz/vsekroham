...
$arNavParams = array(
	"nPageSize" => $arParams["PAGE_REVIEWS_COUNT"],
	...
	if($arParams["ELEMENT_ID"] > 0)
	{
		...
		if($obElement = $rsElement->GetNextElement())
		{
			...
			if (CModule::IncludeModule("highloadblock")) {
				$limit = $arNavigation["SIZEN"]; //отзывов на странице
				...
				$arResult["NAV_STRING"] = $result->GetPageNavStringEx($navComponentObject, '', 'reviews', 'N');
...
if(isset($arResult["ID"]))
{
	...
	if ($numPage > 1) {
		$APPLICATION->SetTitle(($numPage>1?'Отзывы покупателей. '.$arResult["NAME"].' - страница №'.$numPage:$arResult["NAME"]));
	}
	else {
