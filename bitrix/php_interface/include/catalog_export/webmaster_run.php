...
if (empty($arRunErrors))
{
	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		$filter = Array("IBLOCK_ID"=>$IBLOCK_ID, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "!PROPERTY_AVAILABLE"=>false);
		if (!$bAllSections)
		{
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			if ($arAcc["PROPERTIES"][67]["VALUE"] == "Y") {
				$str_AVAILABLE = ' available="false"';
			}
			else {
				$str_AVAILABLE = ' available="true"';
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			/*if (intval($minPrice) < 2000) {
				$strTmpOff_tmp.="<delivery>true</delivery>\n<local_delivery_cost>350</local_delivery_cost>\n";
			}
			else {
				$strTmpOff_tmp.="<delivery>true</delivery>\n<local_delivery_cost>1</local_delivery_cost>\n";
			}*/
			...
			if (intval($minPrice) < 2000) {
				$strTmpOff_tmp.="<local_delivery_cost>350</local_delivery_cost>\n";
			}
			else {
				$strTmpOff_tmp.="<local_delivery_cost>0</local_delivery_cost>\n";
			}
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					if ($arItem["PROPERTIES"]["NOT_AVAILABLE"]["VALUE"] == "Y") {
						$arOfferItem["IS_AVAILABLE"] = 'false';
					}
					else {
						$arOfferItem["IS_AVAILABLE"] = 'true';
					}
					...
					$strOfferYandex.="<delivery>true</delivery>\n";
					if (intval($minPrice) < 2000) {
						$strOfferYandex.="<local_delivery_cost>350</local_delivery_cost>\n";
					}
					else {
						$strOfferYandex.="<local_delivery_cost>0</local_delivery_cost>\n";
					}
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
			...
			if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers)
			{
				...
				if ($arItem["PROPERTIES"][67]["VALUE"]=="Y") {
					$str_AVAILABLE = ' available="false"';
				}
				else {
					$str_AVAILABLE = ' available="true"';
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
	...
	@fwrite($fp, "<local_delivery_cost>0</local_delivery_cost>\n");
