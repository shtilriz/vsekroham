...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					$strOfferYandex .= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."utm_source=nadavi&amp;utm_medium=cpc&amp;utm_campaign=price2</url>\n";
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				$strOfferYandex.= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]).(strstr($arItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."utm_source=nadavi&amp;utm_medium=cpc&amp;utm_campaign=price2</url>\n";
