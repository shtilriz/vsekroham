...
function clearDayStr ($data) {
	$arDaysStr = array('день', 'дня', 'дней');
	$str = $data;
	foreach ($arDaysStr as $value) {
		$str = str_replace($value, "", $str);
	}
	$arDays = explode("-", $str);
	if (count($arDays) > 1) {
		if (abs($arDays[1] - $arDays[0]) > 2) {
			$arDays[0] = $arDays[1] - 2;
			$str = $arDays[0].'-'.$arDays[1];
		}
	}
	return $str;
}
function getDeliveryOptions ($PRODUCT_ID, $CITY) {
	$delivOptStr = '';
	if ($PRODUCT_ID && $CITY) {
		if ($arBestDelivery = getBestDelivery($PRODUCT_ID, $CITY)) {
			$delivOptStr = "<delivery-options><option cost=\"".$arBestDelivery["price"]."\" days=\"".trim(clearDayStr($arBestDelivery["day"]))."\" order-before=\"19\"/></delivery-options>\n";
		}
	}
	return $delivOptStr;
}
...
if (empty($XML_FILE_NAME)) {
	$arRunErrors[] = "Не указаны города выгрузки";
}
...
$strSalesNotes = 'Доставка после 100% предоплаты';

foreach ($XML_FILE_NAME as $kCity => $SETUP_FILE_NAME) {
	if (!$SETUP_FILE_NAME) {
		continue;
	}
	$SETUP_FILE_NAME = "/bitrix/catalog_export/region/".$SETUP_FILE_NAME;
	...
	if (empty($arRunErrors))
	{
		...
		if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
		{
			...
			while ($obElement = $res->GetNextElement())
			{
				...
				$strTmpOff_tmp .= getDeliveryOptions ($arAcc["ID"], $CITY[$kCity]);
				...
				foreach ($arYandexFields as $key)
				{
					switch ($key)
					{
						...
						case 'description':
							...
							break;
						case 'sales_notes':
							$strTmpOff .= "<sales_notes>".yandex_text2xml($strSalesNotes, true)."</sales_notes>\n";
							break;
			...
			while ($obItem = $rsItems->GetNextElement())
			{
				...
				if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND'])
				{
					...
					if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
					{
						...
						$strOfferYandex = '';
						$strOfferYandex .= "<offer id=\"".$arOfferItem["ID"]."\"".$str_TYPE.">\n";
						...
						$strOfferYandex .= getDeliveryOptions ($arItem["ID"], $CITY[$kCity]);
						...
						foreach ($arYandexFields as $key)
						{
							...
							case 'description':
								...
								break;
							case 'sales_notes':
								$strOfferYandex .= "<sales_notes>".yandex_text2xml($strSalesNotes, true)."</sales_notes>\n";
								break;
							case 'country_of_origin':
				...
				if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers)
				{
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'description':
							...
							break;
						case 'sales_notes':
								$strValue .= "<sales_notes>".yandex_text2xml($strSalesNotes, true)."</sales_notes>\n";
							break;
						case 'country_of_origin':
							if ($arItem["PROPERTIES"][6]["VALUE"]) {
								if (array_key_exists($arItem["PROPERTIES"][6]["VALUE"], $arMakersCountry)) {
