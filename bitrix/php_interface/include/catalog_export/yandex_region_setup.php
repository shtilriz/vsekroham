<?
//<title>Yandex Market для регионов</title>
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/catalog/export_setup_templ.php');
$APPLICATION->AddHeadScript('//yastatic.net/jquery/2.1.4/jquery.min.js');
...
$strAllowExportPath = "/bitrix/catalog_export/region/";
...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
	...
	if (array_key_exists('SHOW_PICKUP', $arOldSetupVars))
		$SHOW_PICKUP = $arOldSetupVars['SHOW_PICKUP'];
	if (array_key_exists('CITY', $arOldSetupVars))
		$CITY = $arOldSetupVars['CITY'];
	if (array_key_exists('XML_FILE_NAME', $arOldSetupVars))
		$XML_FILE_NAME = $arOldSetupVars['XML_FILE_NAME'];
...
if ($STEP>1)
{
	...
	if (preg_match(BX_CATALOG_FILENAME_REG, $strAllowExportPath.$SETUP_FILE_NAME))
	{
...
?>
<tr>
	<td width="40%">Выводить тег pickup:</td>
	<td width="60%"><input type="checkbox" name="SHOW_PICKUP" value="Y"<?=($SHOW_PICKUP=='Y'?' checked':'')?>></td>
</tr>
<tr>
	<td width="40%" valign="top">Города выгрузки:</td>
	<td width="60%" id="city-row">
		<?if (empty($CITY)):?>
		<div class="city-row-block">
			<select name="CITY[]">
				<option value=""></option>
			...
			<label>файл: <b>/bitrix/catalog_export/region/</b> <input type="text" name="XML_FILE_NAME[]" value=""></label>
		</div>
		<?else:?>
			...
			<div class="city-row-block">
				<select name="CITY[]">
					<option value=""></option>
					<?foreach ($arCity as $value) {
						echo '<option value="'.$value.'"'.($city==$value?' selected':'').'>'.$value.'</option>';
					}?>
				</select>
				<label>файл: <b>/bitrix/catalog_export/region/</b> <input type="text" name="XML_FILE_NAME[]" value="<?=$XML_FILE_NAME[$key]?>"></label>
			</div>
			<?endforeach;?>
		<?endif;?>
	</td>
...
<tr>
	<td width="40%"></td>
	<td width="60%">
		<input type="button" value="Добавить город" id="addCity">
	</td>
</tr>
<?
}
...
if (2 > $STEP)
{
	...
	<input type="hidden" name="SETUP_FIELDS_LIST" value="V,IBLOCK_ID,SETUP_SERVER_NAME,SETUP_FILE_NAME,XML_DATA,SHOW_PICKUP,CITY,XML_FILE_NAME">
...
$(function() {
	$('#addCity').on('click', function (e) {
		e.preventDefault();
		$('#city-row .city-row-block:first').clone().appendTo('#city-row');
	});
});
</script>